...
export class CorporateController {
  ...
  async register(@Request() req: any, @Body() body: { name: string; type: string; email: string }) {
    return this.corporateService.createOrganization(req.user.sub, body);
  }

  @Post('license/pay')
  async licensePay(@Request() req: any, @Body() body: { plan: string }) {
    const membership = await this.corporateService.getUserOrg(req.user.sub);
  ...
  async acceptInvite(@Request() req: any, @Body() body: { token: string }) {
    return this.corporateService.acceptInvite(body.token, req.user.sub);
  }
  ...
  async me(@Request() req: any) {
    return this.corporateService.getUserOrg(req.user.sub);
  }
}
